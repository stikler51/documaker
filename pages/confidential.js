...
class Confidential extends React.Component {
  ...
  async langHandler(val) {
    ...
    const page = await fetch(`http://sandbox.webbro.pro/api/static_pages?lang=${val}&id=7`);
...
Confidential.getInitialProps = async (props) => {
  let lang = cookies(props).lang || props.store.getState().lang;

  //http://sandbox.webbro.pro/api/static_pages?lang=ru&id=7

  const page = await fetch(`http://sandbox.webbro.pro/api/static_pages?lang=${lang}&id=7`);
  let pageJson = await page.json();
...
export default connect(mapStateToProps, mapDispatchToProps)(Confidential);